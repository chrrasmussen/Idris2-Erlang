...
export
genMkIO : {vars : _} -> Line -> ErlExpr vars -> ErlExpr vars
genMkIO l expr =
  let worldVar = MN "" 0
...
export
genSequence : {vars : _} -> Line -> (exprs : List (ErlExpr vars)) -> {auto prf : NonEmpty exprs} -> ErlExpr vars
...
export
genMainInit : {argsVar, vars : _} -> Line -> ErlExpr (argsVar :: vars) -> ErlExpr (argsVar :: vars)
genMainInit l expr =
...
export
genStringToDouble : {vars : _} -> Line -> ErlExpr vars -> ErlExpr vars
