...
addField (PLibrary e)        pkg = pure $ { library := Just e } pkg
...
addDeps pkg = traverse_ (\p => addPkg p.pkgname) (depends pkg)
...
build pkg opts
    = do assertIdrisCompatibility pkg
         ...
         whenJust (library pkg) $ \ libName =>
           do let packageNamespaces = map fst (modules pkg)
              compileLibHelper libName packageNamespaces changedModules
